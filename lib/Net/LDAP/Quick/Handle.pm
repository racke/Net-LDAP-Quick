...
sub base {
    ...
    if (@_) {
        # prepend path
        return join(',', @_, $self->{_quick_settings}->{base});
    }
    return $self->{_quick_settings}->{base};
}
...
sub rebind {
    ...
    $ldret = $self->bind($self->{_quick_settings}->{bind},
                         password => $self->{_quick_settings}->{password});
    if ($ldret->code) {
        return;
    }
    return $self;
